...
exports.handleRequest = function(req, res, cb) {
    ...
    var parts = servletUrl.query.id.replace('http://', '').split(':'),
        port = parts[1],
        host = parts[0];
    ...
    return store.getNode(host, port)
        .then(function(node) {
            if (!node) {
                return new models.JsonResponse(404, {
            ...
            return store.updateNode(node)
                .then(function() {
                    ...
                    return new models.JsonResponse(200, {msg: 'Proxy found!', success: true});
                });
        })
        .then(cb)
        .done();
