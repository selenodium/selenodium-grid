...
var process = function(req, node, type, protocolCallback, cb, retries, protocol, res) {
    ...
    if (protocol === 'RC') {
        registry.getSessionById(parameters['sessionId'], function(err, session) {
        ...
        registry.getSessionById(webdriverServlet.extractSessionId(req.url), function(err, session) {
            session.lastSentTime = (new Date()).getTime();
