...
var DEFAULT_NEW_SESSION_WAIT_TIMEOUT = -1;
...
var SessionQueue = inherit({
    ...
    addRequest: function(req) {
        var self = this,
            r = new SessionRequest(req);

        self.requests.add(r);

        // remove pending request from the queue on client disconnect
        req.node.on('close', function() {
            var msg = util.format('Request removed from the queue because of client disconnect (%s)', r.getId());

            // reject previously saved defer
            r.defer.reject(new Error(msg));
            log.warn(msg);
