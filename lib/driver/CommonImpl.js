...
module.exports = inherit(/** @lends CommonImpl.prototype */ {
    ...
    getNewSessionFromNode: function(req, node) {
        ...
        log.debug('Sending new session request to node with ID=%s; capabilities: %j', node.getId(), req.capabilities);
        return self.proxyRequestToNode(req, node)
            .then(function(res) {
                log.debug('Got new session response from node with ID=%s', node.getId());
            ...
            .spread(function(sessionId, resCaps) {
                return {
                    ...
                    desiredCapabilities: req.capabilities
                }
            })
            .catch(function(err) {
